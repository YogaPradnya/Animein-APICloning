...
3. [Pencarian Anime](#3-pencarian-anime) 🆕 Filter Genre & Sorting
4. [List Genre](#4-list-genre) 🆕
5. [List Anime](#5-list-anime)
6. [Video Episode](#6-video-episode)
7. [Info Anime (AnimeInWeb)](#7-info-anime-animeinweb)
8. [Video Episode (AnimeInWeb)](#8-video-episode-animeinweb)
9. [Jadwal Anime](#9-jadwal-anime)
10. [Anime Trending](#10-anime-trending)
11. [Anime Baru](#11-anime-baru)
12. [Anime Hari Ini](#12-anime-hari-ini)
13. [Download Episode](#13-download-episode)
14. [Batch Download Info](#14-batch-download-info) 🆕 25 eps per pack
15. [Download Batch](#15-download-batch)
...
Mencari anime dengan fitur filter genre dan sorting. Default sorting adalah berdasarkan views (paling populer di atas).

### Endpoint
```
GET /api/v1/search?q={keyword}&genre={id}&sort={sortBy}&page={page}
```
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `q` | string | No | Keyword pencarian (judul anime) |
| `genre` | number | No | ID genre untuk filter (lihat endpoint `/genres`) |
| `sort` | string | No | Sorting: `views` (default), `title`, `favorites`, `newest` |
| `page` | number | No | Halaman (default: 0) |

### Sorting Options

| Sort | Description |
|------|-------------|
| `views` | Paling populer (views tertinggi) - **DEFAULT** |
| `title` | Urut berdasarkan nama A-Z |
| `favorites` | Paling difavoritkan |
| `newest` | Paling baru |
...
# Filter genre Action (id: 14), sort by views (paling populer)
curl "http://localhost:3000/api/v1/search?genre=14&sort=views"

# Filter genre Romance, sort by title
curl "http://localhost:3000/api/v1/search?genre=20&sort=title"

# Kombinasi keyword + genre + sort
curl "http://localhost:3000/api/v1/search?q=hero&genre=14&sort=favorites"
```

**JavaScript:**
```javascript
// Search anime action paling populer
const response = await fetch('http://localhost:3000/api/v1/search?genre=14&sort=views');
const data = await response.json();

// Tampilkan 5 anime action terpopuler
data.data.slice(0, 5).forEach((anime, i) => {
  console.log(`${i+1}. ${anime.title} - ${anime.views.toLocaleString()} views`);
});

// Filter dengan keyword
const searchResponse = await fetch('http://localhost:3000/api/v1/search?q=naruto&sort=views');
...
Mendapatkan daftar semua genre yang tersedia untuk filter pencarian.
...
| 3 | Demon | 23 | Seinen |
| 4 | Drama | 24 | Shoujo |
| 5 | Ecchi | 25 | Shoujo Ai |
| 6 | Fantasy | 26 | Shounen |
| 7 | Game | 27 | Shounen Ai |
| 8 | Harem | 28 | Slice of Life |
| 9 | Historical | 29 | Sports |
| 10 | Horror | 30 | Super Power |
| 11 | Magic | 31 | Supernatural |
| 12 | Martial Arts | 32 | Thriller |
| 13 | Mecha | 33 | Tokusatsu |
| 15 | Military | | |
| 16 | Music | | |
| 17 | Mystery | | |
| 18 | Parody | | |
...
**JavaScript:**
```javascript
// Ambil list genre untuk dropdown filter
const response = await fetch('http://localhost:3000/api/v1/genres');
const data = await response.json();

// Tampilkan sebagai options
data.data.forEach(genre => {
  console.log(`<option value="${genre.id}">${genre.name}</option>`);
});
```
...
## 14. Batch Download Info

Mendapatkan informasi pembagian batch download per 25 episode. Berguna untuk anime dengan banyak episode seperti One Piece, Naruto, dll.

### Endpoint
```
GET /api/v1/download/batch-info?animeId={id}
```
...
```json
{
  ...
  "data": {
    ...
    "batches": [
      {
        "packNumber": 1,
        "label": "Pack 1: Episode 1-25",
        "startEpisode": 1,
        "endEpisode": 25,
        "episodeCount": 25,
        "downloadUrl": "/api/v1/download/batch?animeId=426&startEpisode=1&endEpisode=25"
      },
      {
        "packNumber": 2,
        "label": "Pack 2: Episode 26-50",
        "startEpisode": 26,
        "endEpisode": 50,
        "episodeCount": 25,
        "downloadUrl": "/api/v1/download/batch?animeId=426&startEpisode=26&endEpisode=50"
      }
    ]
  }
}
...
**cURL:**
```bash
curl "http://localhost:3000/api/v1/download/batch-info?animeId=426"
```

**JavaScript:**
```javascript
const animeId = '426'; // One Piece

// Ambil info batch
const response = await fetch(`http://localhost:3000/api/v1/download/batch-info?animeId=${animeId}`);
const data = await response.json();

console.log(`Anime: ${data.data.animeTitle}`);
console.log(`Total Episode: ${data.data.totalEpisodes}`);
console.log(`Total Pack: ${data.data.totalPacks}`);

// Tampilkan semua pack
data.data.batches.forEach(batch => {
  console.log(`${batch.label}`);
  // Download URL untuk pack ini
  console.log(`  URL: ${batch.downloadUrl}`);
});

// Contoh: Download pack ke-1 dengan resolusi 720p
const pack1 = data.data.batches[0];
const downloadUrl = `${pack1.downloadUrl}&resolution=720p`;
console.log(`Download Pack 1: ${downloadUrl}`);
```
...
```
1. User pilih anime
2. Panggil /api/v1/download/batch-info?animeId=...
3. Tampilkan dropdown/list pack yang tersedia:
   - Pack 1: Episode 1-25
   - Pack 2: Episode 26-50
   - Pack 3: Episode 51-75
   - ...
4. User pilih pack dan resolusi
5. Panggil /api/v1/download/batch dengan startEpisode dan endEpisode dari pack
6. Tampilkan link download per episode
```
